<template>
    <v-card>
        ...
        <v-card-text>
             <vue-bootstrap4-table :rows="contributions" :columns="columns" :config="config" >
        ...
        <edit-contribution :dialog="dialog" :contribution="contribution" @close="close"  @contribution="update"></edit-contribution>

    </v-card>
</template>
<script>
import VueBootstrap4Table from 'vue-bootstrap4-table';
import EditContribution from './Edit.vue';
export default {
    data:()=>({
        contributions:[],
        contribution:{},
    ...
    methods:{
        ...
        edit (item) {
            ...
            axios.get(`/api/auth/contribution/${item.id}/edit`)
            .then(response => {
        ...
        update (item) {
            console.log(item);
            axios.post('/api/auth/contribution', item)
                  .then(response => {
                        iziToast.success({
                            title: 'Registro Satisfactorio',
                            message: 'Se registro '+response.data.afp_name,
                        });
                        this.search();
                    })
        ...
        destroy (item) {
            ...
            axios.delete(`/api/auth/contribution/${item.id}`)
            .then((response)=>{
