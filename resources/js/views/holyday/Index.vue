<template>
    <v-card>
        ...
        <v-card-text>
             <vue-bootstrap4-table :rows="holydays" :columns="columns" :config="config" >
        ...
        <edit-holyday :dialog="dialog" :holyday="holyday" @close="close"  @holyday="update"></edit-holyday>

    </v-card>
</template>
<script>
import VueBootstrap4Table from 'vue-bootstrap4-table';
import EditHolyday from './Edit.vue';
export default {
    data:()=>({
        holydays:[],
        holyday:{},
    ...
    methods:{
        search(){
            axios.get('/api/auth/holyday')
                 .then((response)=>{
                    this.holydays = response.data.holydays;
                });
        },
        create() {
            this.holyday ={};
        ...
        edit (item) {
            ...
            axios.get(`/api/auth/holyday/${item.id}/edit`)
            .then(response => {
                this.holyday = response.data.holyday
            })
            .catch(error => {
                console.log(error);
            });
        ...
        update (item) {
            console.log(item);
            axios.post('/api/auth/holyday', item)
                  .then(response => {
                        iziToast.success({
        ...
        destroy (item) {
            ...
            axios.delete(`/api/auth/holyday/${item.id}`)
            .then((response)=>{
