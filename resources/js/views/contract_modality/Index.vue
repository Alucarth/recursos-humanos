<template>
    <v-card>
        <v-card-title>
            <h3>Modalidades de Contrato</h3>
        <v-spacer></v-spacer>
        <v-btn @click="create();" color="primary" dark class="mb-2">Nuevo</v-btn>
        </v-card-title>
        <v-card-text>
             <vue-bootstrap4-table :rows="contract_modalities" :columns="columns" :config="config" >
        ...
        <edit-contract :dialog="dialog" :contract_modality="contract_modality" @close="close"  @contract_modality="update"></edit-contract>
...
export default {
    ...
    methods:{
        ...
        create() {
            this.contract_modality ={};
            this.dialog = true;
        },

        edit (item) {

            axios.get(`/api/auth/contract_modality/${item.id}/edit`)
            .then(response => {
                this.contract_modality = response.data.contract_modality
            })
            .catch(error => {
                console.log(error);
            });
        ...
        update (item) {
            console.log(item);
            axios.post('/api/auth/contract_modality', item)
                  .then(response => {
                        iziToast.success({
        ...
        destroy (item) {
            ...
            axios.delete(`/api/auth/contract_modality/${item.id}`)
            .then((response)=>{
